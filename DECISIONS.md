...
| **Eligibility tracking (revised)** | Dropped the one-shot `playFromStartEligible` ref. CountdownOverlay now fires whenever `engine.position === 0` at play time, with no per-song "used up" state. | Ad hoc UI fix: users expected the countdown on every replay-from-start (resume-after-pause-at-0, seek-to-0-then-play, replay after song ends), not just the very first play of a song. |
| **Arrow-key seeks route through context `seek()`** | `seek` callback defined above the keyboard effect; ArrowLeft/Right call `seek(position ± 5)` instead of `engine?.seek(...)`. | Ensures keyboard seeks also cancel any active pad countdown, consistent with all other seek paths. |
...
### #131 — warn at points of use when ffmpeg/GPU are missing

| Choice | Decision | Why |
|---|---|---|
| **Two renderer-reachable enqueue points, both covered** | `importQueue.ts` has `startImport()` (wired to `ImportDialog`'s Add button) and `retryImport()` (wired to the Retry button on a failed `SongCard`/`SongRow`, which bypasses the dialog entirely). An earlier draft of this fix only covered the dialog; corrected after review to also guard `useSongCardActions`' `retry` handler. | Retry re-running the exact same doomed job with no check was the same silent-failure bug the issue exists to fix — just reachable from a different button. |
| **Status lives in `LibraryContext`, not per-component** | `usePipelineStatus()` is called once in `LibraryProvider` and exposed as `pipelineStatus`; `ImportDialog`, `SongCard`, and `SongRow` all read it from `useLibraryContext()` instead of each fetching their own copy. | One fetch shared by every point of use under Library, instead of N redundant IPC round-trips. Still "fresh, not stale": `LibraryProvider` remounts (and refetches) every time the user navigates away to Settings and back. |
| **Retry blocked silently → disabled button + tooltip, not a dialog** | When blocked, the Retry button gets `disabled` + a `title` tooltip with the same `import.pythonMissing`/`ffmpegMissing` text as the dialog banner, instead of opening a new dialog. | Matches this exact file's existing idiom — `song.error` already surfaces via a disabled-state tooltip on the same button family — rather than introducing a new UI pattern for one icon button. |
| **GPU advisory suppressed while blocking (no ffmpeg/python)** | `PipelineRequirementBanner` only shows the GPU advisory line when `status.ready` is true. | Avoids noise — if the job can't run at all yet, "it'll be slow" is moot until the blocking issue is fixed. |
