...
| Choice | Decision | Why |
|---|---|---|
| **One banner covers both "Add Song" and "stem separation"** | Issue's acceptance criteria list these as two separate points of use, but `startImport()` (`importQueue.ts`) enqueues a single job that runs download *and* separation together — there's no separate user-triggered "start separation" action to attach a second warning to. `PipelineRequirementBanner` in `ImportDialog.tsx` (the one enqueue surface) satisfies both criteria. | Building a second, currently-nonexistent UI surface just to have two banners would be inventing scope the codebase doesn't have a hook for. |
...
| **Status fetched fresh per dialog mount, no caching** | New `usePipelineStatus()` hook re-fetches `pipeline.status()` every time `ImportDialog` mounts, instead of reusing `App.tsx`'s first-run-gate snapshot. | Acceptance criteria explicitly requires reflecting *current* status, not a stale one from before the user skipped setup. |
