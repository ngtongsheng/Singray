...
> **Now → EL4**, then top-to-bottom. Phase order = execution order chosen in grilling: safety/quick bugs → shared primitives → nav redesign → feature views → polish. Phase 0 (Round 1 verification) is env-blocked / user-side and doesn't block the coding pointer.
...
- 2026-06-14 · EL3 · Removed the centered `timing.done` banner from TimingStep. New full-width strip directly under `WaveformStrip` (mirrors Library's import strip: `border-b bg-surface px-6 py-1.5` + absolute `h-0.5 bg-accent` fill bar at `width: progressPct%`) shows `stamps.length/flatUnits.length` + `progressPct%` while in progress, and the existing `timing.done` text ("全部打完 — 可以预览了") once `done`. The large current-line focus display is now unconditional (previously hidden behind the `done` banner) — when done it naturally renders the last line fully in `text-lyric-sung`. No new translation keys. Verified via playwright on a fully-timed song: strip shows the done message with a full-width accent fill, centered banner gone, focus display still shows the last line in sung color. `npm run check` green. **[x]**.
