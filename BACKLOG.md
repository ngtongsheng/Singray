...
> **Now → EL3**, then top-to-bottom. Phase order = execution order chosen in grilling: safety/quick bugs → shared primitives → nav redesign → feature views → polish. Phase 0 (Round 1 verification) is env-blocked / user-side and doesn't block the coding pointer.
...
- 2026-06-14 · SNG3 · Investigated: Ken Burns (R1.4) was running continuously but its amplitude/duration (scale 1.10→1.22 over 60s) made it read as static through `blur-3xl`. Tuned `@keyframes ken-burns` to scale 1.08→1.28 over a 45s cycle (still smooth ease-in-out) for visible-but-subtle drift. Also changed the pause gate from `windowHidden` only to `windowHidden || !playing` (Player.tsx), per the decision — background now holds still while paused, animates during playback. Verified via playwright: `getComputedStyle(...).transform` is static while paused, changes meaningfully (scale 1.080→1.113, translate ~9px) over 4s of playback; `PerformanceObserver({type:'layout-shift'})` recorded 0 entries during steady playback (transform-only animation). `npm run check` green. **[x]**.
