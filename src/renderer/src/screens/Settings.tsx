...
function SeparationModelSelect({
  ...
  const load = useCallback(async (force = false): Promise<void> => {
    ...
    try {
      ...
      setModels(list)
      // If current value isn't in the list, keep it (user might have typed a custom one)
    } catch {
      // Fallback: keep whatever we have or default
        ...
        options={
          loading
            ...
            : [
                ...
                // If current value is custom/unknown, show it as an option too
                ...(!valid && value
