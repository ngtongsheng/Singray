...
def cmd_list_models(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # audio-separator CLI unavailable — return just the default
        emit({"stage": "done", "models": [DEFAULT_MODEL]})
        return 0

    # Parse tabular CLI output: extract model filenames (first column)
    models: list[str] = []
...
def cmd_clear_models(args: argparse.Namespace) -> int:
    """Remove all downloaded models from the models directory (for testing redownload)."""
    import shutil
